...
deleted, and no payload may grant or strip its membership (privilege escalation / neutralizing break-glass). Membership
changes happen only via seeder or console. Payloads that keep membership as-is pass, so a super admin's other roles stay
editable.

## Lockout invariants

Every access mutation goes through one write path (`AccessControlService`): a single transaction serialized on the
lockout permission rows, verified against two invariants before commit, independently for each configured lockout
permission:
...
queries and veto single-record access. A project with none gets plain RBAC plus the rules. How the layers compose, a
...
- **Users**: browse/search/export (columns and page size configurable), account facts editing (name, email verification,
  ...
  admins and lockout-permission holders — are refused unless the actor is a super admin. Deleted accounts stay reachable
...
| `lockout_permissions`                                          | `users.manage`, `roles.manage`     | The capabilities the invariants protect.                                                                                                                 |
