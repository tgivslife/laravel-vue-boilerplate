...
| `script-src`                             | `'self' 'nonce-…'`       | Per-response nonce, stamped on `@vite` tags, the boot script in `app.blade.php`, and the Horizon and Telescope inline bundles. No `unsafe-inline`, no `unsafe-eval` - the SPA is precompiled SFCs. See tiers below.                        |
...
| `/telescope/*`  | `'self' 'nonce-…' 'unsafe-eval'`       | Same shape: Telescope 5.22+ takes the nonce through `Telescope::cspNonce()` (guarded by `class_exists`, the package is require-dev). Only eval is added. |
...
out across many emails so no single email's failure bucket ever trips the lockout. The ceiling is sized by
`LOGIN_REQUEST_MAX_ATTEMPTS` per `LOGIN_REQUEST_DECAY_MINUTES` (20 per minute by default); keep it well above a human's
rate so shared egress IPs (offices, CGNAT) are not caught. The lockout's knobs (`LOGIN_LOCKOUT_*`) and every other
limiter's live with their features.
